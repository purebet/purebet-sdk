...
export function decodeMkt(
   ...
   home: string = "Home",
   away: string = "Away"
): {name: string, type: string, description: string, sides: [string, string]|[true, null], value?: number|string} | undefined {
   if(mkt == 0){
      return {name: "Moneyline", type: "ML", description: "Moneyline", sides: [home, away]};
   } else if(mkt == 1){
      return {name: "Home Win", type: "1X2", description: "Home Win in Regular Time", sides: ["Home win", away+"/Draw"]};
   } else if(mkt == 2){
      return {name: "Draw", type: "1X2", description: "Draw in Regular Time", sides: ["Draw", home+"/Away"]};
   } else if(mkt == 3){
      return {name: "Away Win", type: "1X2", description: "Away Win in Regular Time", sides: ["Away win", "Home/Draw"]};
   } else if(mkt == 4){
      return {name: "Both Teams To Score", type: "BTTS", description: "Both Teams To Score", sides: ["Yes", "No"]};
   } else if (mkt >= 10 && mkt <= 50) {
      return {name: `Outcome ${mkt-10}`, type: "MSO", description: "Multi-Selection Outcome", value: mkt-10, sides: ["Win", "Not win"]};
   } else if (mkt >= 100 && mkt <= 300) {
      return {name: `Handicap ${(mkt-200)/2}`, type: "AH", description: "Handicap", value: (mkt-200)/2, sides: [home, away]};
   } else if (mkt >= 400 && mkt <= 450) {
      return {name: `Both Teams To Score (Yes) & Over ${(mkt-400)/2}`, type: "BTTS+OU", description: "Both Teams To Score (Yes) & Over/Under", value: (mkt-400)/2, sides: [true, null]};
   } else if (mkt >= 450 && mkt <= 500) {
      return {name: `Both Teams To Score (Yes) & Under ${(mkt-450)/2}`, type: "BTTS+OU", description: "Both Teams To Score (Yes) & Over/Under", value: (mkt-450)/2, sides: [true, null]};
   } else if (mkt >= 500 && mkt <= 550) {
      return {name: `Both Teams To Score (No) & Over ${(mkt-500)/2}`, type: "BTTS+OU", description: "Both Teams To Score (No) & Over/Under", value: (mkt-500)/2, sides: [true, null]};
   } else if (mkt >= 550 && mkt <= 600) {
      return {name: `Both Teams To Score (No) & Under ${(mkt-550)/2}`, type: "BTTS+OU", description: "Both Teams To Score (No) & Over/Under", value: (mkt-550)/2, sides: [true, null]};
      ...
      return {name: `${HTWinner}/${FTWinner}`, type: "HTFT", description: "Half Time/Full Time", value: `${HTWinner}/${FTWinner}`, sides: [true, null]};
      ...
      return {name: `${FTWinner} & ${BTTS}`, type: "FT+BTTS", description: "Full Time & Both Teams To Score", value: `${FTWinner}/${BTTS}`, sides: [true, null]};
   } else if (mkt >= 700 && mkt <= 750) {
      const HTWinner = numberToSide(parseInt(mkt.toString().substring(1, 2)), home, away);//the 2nd digit
      const FTWinner = numberToSide(parseInt(mkt.toString().substring(2, 3)), home, away);//the 3rd digit
      if(HTWinner === "Unknown" || FTWinner === "Unknown"){
         throw new Error("Invalid market, 2nd and 3rd digit must be 1, 2, or 3 (Home, Draw, Away)");
      ...
      return {name: `Over/Under ${(mkt-1000)/2}`, type: "OU", description: "Over/Under", value: (mkt-1000)/2, sides: ["Over", "Under"]};
   } else if (mkt >= 2000 && mkt <= 3000) {
      return {name: `${home} over/under ${(mkt-2000)/2}`, type: "TOU", description: "Home over/under", value: (mkt-2000)/2, sides: ["Over", "Under"]};
   } else if (mkt >= 3000 && mkt <= 4000) {
      return {name: `${away} over/under ${(mkt-3000)/2}`, type: "TOU", description: "Away over/under", value: (mkt-3000)/2, sides: ["Over", "Under"]};
   } else if (mkt >= 5000 && mkt <= 5500) {
      return {name: `${home} win & over ${(mkt-5000)/2}`, type: "FT+OU", description: "Home win & over/under", value: (mkt-5000)/2, sides: [true, null]};
   } else if (mkt >= 5500 && mkt <= 6000) {
      return {name: `${home} win & under ${(mkt-5500)/2}`, type: "FT+OU", description: "Home win & over/under", value: (mkt-5500)/2, sides: [true, null]};
   } else if (mkt >= 6000 && mkt <= 6500) {
      return {name: `Draw & over ${(mkt-6000)/2}`, type: "FT+OU", description: "Draw & over/under", value: (mkt-6000)/2, sides: [true, null]};
   } else if (mkt >= 6500 && mkt <= 7000) {
      return {name: `Draw & under ${(mkt-6500)/2}`, type: "FT+OU", description: "Draw & over/under", value: (mkt-6500)/2, sides: [true, null]};
   } else if (mkt >= 7000 && mkt <= 7500) {
      return {name: `${away} win & over ${(mkt-7000)/2}`, type: "FT+OU", description: "Away win & over/under", value: (mkt-7000)/2, sides: [true, null]};
   } else if (mkt >= 7500 && mkt <= 8000) {
      return {name: `${away} win & under ${(mkt-7500)/2}`, type: "FT+OU", description: "Away win & over/under", value: (mkt-7500)/2, sides: [true, null]};
      ...
      return {name: `Correct Score ${home}-${away}`, type: "CS", description: "Correct Score", value: `${home}-${away}`, sides: [true, null]};
   } else if (mkt >= 11000 && mkt <= 65000) {
      if(mkt === 12000){
         return {name: "To Score", type: "PP", description: "To Score", sides: ["Yes", "No"]};
      }
      if(mkt === 57000){
         return {name: "First Goalscorer", type: "PP", description: "First Goalscorer", sides: ["Yes", "No"]};
      }
      if(mkt === 58000){
         return {name: "To Be Booked", type: "PP", description: "To Be Booked", sides: ["Yes", "No"]};
      }
      ...
      return {name: `${playerPropType} Over/Under ${value}`, type: "PP", description: `Player Prop: ${playerPropType}`, value: value, sides: ["Over", "Under"]};
   }
}
